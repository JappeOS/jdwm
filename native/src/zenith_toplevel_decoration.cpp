...
void toplevel_decoration_create_handle(wl_listener* listener, void* data) {
	...
	auto* xdg_surface_ptr = static_cast<ZenithXdgSurface*>(wlr_toplevel_decoration->toplevel->base->data);
...
void request_mode_handle(wl_listener* listener, void* data) {
	...
	// wlr_xdg_toplevel_decoration_v1_set_mode internally calls
	// wlr_xdg_surface_schedule_configure which asserts surface->initialized.
	// If the client sends request_mode before the initial commit (as Chromium
	// does), we must defer. The mode is applied in zenith_xdg_toplevel_commit.
	if (!wlr_toplevel_decoration->toplevel->base->initialized) {
		return;
	}
...
void destroy_handle(wl_listener* listener, void* data) {
	...
	// wlroots asserts listener lists are empty right after this signal.
	wl_list_remove(&toplevel_decoration->request_mode.link);
	wl_list_remove(&toplevel_decoration->destroy.link);
