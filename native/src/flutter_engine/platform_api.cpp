	...
	server->callable_queue.enqueue([server] {
		if (!server->startup_command.empty()) {
			if (fork() == 0) {
				...
				wlr_log(WLR_ERROR, "Failed to launch startup command '%s': %s",
				        server->startup_command.c_str(), strerror(errno));
				_exit(127);
			}
		}
	});
...
void enable_display(ZenithServer* server, const flutter::MethodCall<>& call,
	...
	server->callable_queue.enqueue([server, enable] {
		server->output_manager->set_display_enabled(enable);
	});

	result->Success();
}

void request_monitors_snapshot(ZenithServer* server, const flutter::MethodCall<>& call,
                               std::unique_ptr<flutter::MethodResult<>>&& result) {
	(void) call;
	server->callable_queue.enqueue([server] {
		if (server->embedder_state != nullptr) {
			server->embedder_state->publish_monitor_layout();
		}
	});
	result->Success();
}
...
void set_cursor_visible(ZenithServer* server, const flutter::MethodCall<>& call,
                        std::unique_ptr<flutter::MethodResult<>>&& result) {
	auto value = std::get<bool>(call.arguments()[0]);
	server->callable_queue.enqueue([server, value] {
...
void lock_cursor(ZenithServer* server, const flutter::MethodCall<>& call,
                 std::unique_ptr<flutter::MethodResult<>>&& result) {
	auto value = std::get<bool>(call.arguments()[0]);
	server->callable_queue.enqueue([server, value] {
		if (server->pointer != nullptr) {
			server->pointer->set_manual_locked(value);
		}
	});
	result->Success();
}
