	...
	platform_method_channel->SetMethodCallHandler(
		  [server](const flutter::MethodCall<>& call, std::unique_ptr<flutter::MethodResult<>> result) {
			  ...
			  if (method_name == "startup_complete") {
				  ...
				  set_cursor_visible(server, call, std::move(result));
			  } else if (method_name == "lock_cursor") {
				  lock_cursor(server, call, std::move(result));
			  } else {
...
void EmbedderState::map_xdg_surface(size_t view_id, int role) {
	callable_queue.enqueue([=] {
