...
void zenith_surface_commit(wl_listener* listener, void* data) {
	...
	// In wlroots 0.19, commits can happen without a buffer (e.g. initial_commit).
	...
	SurfaceRole role;
	if (wlr_xdg_surface_try_from_wlr_surface(surface) != nullptr) {
		role = SurfaceRole::XDG_SURFACE;
	} else if (wlr_subsurface_try_from_wlr_surface(surface) != nullptr) {
		...
		switch (xdg_surface->role) {
			...
			case WLR_XDG_SURFACE_ROLE_POPUP: {
				...
				if (popup->parent != nullptr) {
					assert(wlr_xdg_surface_try_from_wlr_surface(popup->parent) != nullptr);
...
void zenith_surface_destroy(wl_listener* listener, void* data) {
	...
	// wlroots 0.19 asserts all listener lists are empty after destroy.
	wl_list_remove(&zenith_surface->commit.link);
	wl_list_remove(&zenith_surface->new_subsurface.link);
	wl_list_remove(&zenith_surface->destroy.link);
...
void commit_surface(SurfaceCommitMessage* commit_message) {
	...
	// Surface updates must trigger a compositor frame even without input
	// events, otherwise new client content can stay invisible until pointer
	// movement schedules a frame.
